...
interface IL1StandardBridge {
    function depositETHTo(address _to, uint32 minGasLimit, bytes calldata _extraData) external payable;
...
/**
 * @title SwapAndBridgeOptimismRouter
 * @notice Router is a periphery contract that isn't a hook , but a middleman between an EOA
 * and the PoolManager contract.
 * @notice
 */
contract SwapAndBridgeOptimismRouter is Ownable {
    ...
    function swap(
        ...
        // Is user requested a bridge of the output tokens
        ...
        if (settings.bridgeTokens) {
            Currency l1TokenToBridge = params.zeroForOne ? key.currency0 : key.currency1;
     ...
     * @notice Depending on if the user specified they wanted to bridge to the L2 or not
     * through the SwapSettings, we will either take money from PM and send it directly to the recipient
     * on the L1 or take the money to the router contract first and initiate bridge transaction for the recipient on the L2
     */
